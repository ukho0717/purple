...
const GramWrite = ({ user, gram, listLikeGram, commentSubmit, deleteClick, heartClick }) => {
    console.log(gram);
    console.log(user);
    ...
    return(
        <>
            {gram && user && (
                <>
                    <div class="gramP_1">
                        <div className="gramP_1_back" onClick={clickBack}></div><div class="gramP_1_myPho">{<img src={gram.User.profile_pic}/>}</div><span class="gramP_id" id="gramP_1_id">{gram.User.user_nick}</span>
                        {listLikeGram.map(listLike => listLike._id == gram._id && (
                    ...
                    <div class="gramP_3">
                        {gram.likeUser.map(user => (
                            <div>하트</div>
                        ))}
                        {user.likeGram.map(list => list == gram._id ? (
            ...
            <p><span class="gramP_id" id="gramP_3_id">{gram.User.user_nick}</span> <span id="gramP_3_content">{gram.content}</span></p>
                    </div>
                    <div class="gramP_4">
                        {gram.comment.map(comment => (
                            <p><span class="gramP_reID" id="gramP_4_reID">{comment.nickName}</span> {comment.text}</p>
                        ))}
                    </div>
                    <div class="gramP_5">
                    ...
                    <div class="dim-layer2">
                        ...
                        <div id={`${gram._id}layer2`} class="pop-layer2">
                            <div class="pop-container2">
                                <div>
                                <h2>회원 신고</h2>
                                <p><span class="messageP_id">{gram.User.user_nick}</span>님께는 알리지 않습니다.</p>
                                </div>
                                <hr/>
                                <div className="pop_reason">
                                    ...
                                    <button class="popup_close2"><span>취소</span></button>
                                </div>
                            </div>
                        </div>
                    </div>
