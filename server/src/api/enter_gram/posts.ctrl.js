...
export const updateLike = async ctx => {
    ...
    try{
        ...
        if(heart == 'plus'){
            console.log('plus');
            post.update({ likeCount: post.likeCount+1, likeUser: ctx.state.user._id }).exec();
            post.update({ $push: {likeUser: ctx.state.user._id}}).exec();
            ctx.body = post;
        }else{
            console.log('minus');
            post.update({ likeCount: post.likeCount-1 }).exec();
            post.deleteOne({ likeUser: ctx.state.user._id }).exec();
            ctx.body = post;
        }
        // post.update({ likeCount: post.likeCount+1 }).exec();
    }catch(e){
